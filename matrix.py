...
def FIFO(matsize, mem_data, mem_valid, advance_fifo):
    ...
    fifo_out_package = [buf2, buf3, buf4, empty2, empty3, empty4, full] + topbuf

    return buf4, ready, full, fifo_out_package
...
def MMU(acc_mems, data_width, matrix_size, accum_size, vector_in, accum_raddr, accum_waddr, vec_valid, accum_overwrite, lastvec, switch_weights, ddr_data, ddr_valid):  #, weights_in, weights_we):
    ...
    # FIFO
    weights_tile, tile_ready, full, fifo_out_package = FIFO(matsize=matrix_size, mem_data=ddr_data, mem_valid=ddr_valid, advance_fifo=done_programming)
    ...
    return accout, done, addrout, weights_tile, mouts, mma_package, fifo_in_package, fifo_out_package
...
def MMU_top(acc_mems, data_width, matrix_size, accum_size, ub_size, start, start_addr, nvecs, dest_acc_addr, overwrite, swap_weights, ub_rdata, accum_raddr, weights_dram_in, weights_dram_valid):
    ...
    acc_out, done, addrout, weights_tile, mouts, mma_package, fifo_in_package, fifo_out_package = MMU(acc_mems=acc_mems, data_width=data_width, matrix_size=matrix_size, accum_size=accum_size, vector_in=ub_rdata, accum_raddr=accum_raddr, accum_waddr=accum_waddr, vec_valid=vec_valid, accum_overwrite=overwrite_reg, lastvec=last, switch_weights=swap_reg, ddr_data=weights_dram_in, ddr_valid=weights_dram_valid)

    #probe(ub_raddr, "ub_mm_raddr")

    return ub_raddr, acc_out, busy, done, accum_waddr_wv, vec_valid, overwrite_wv, addrout, weights_tile, mouts, mma_package, fifo_in_package, fifo_out_package
