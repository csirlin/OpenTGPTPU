...
# run a squish test with chosen bitwidth, matsize, and number of weights in the
# queue after setup for different distances. 
...
def min_viable_distance_fixed(function, bitwidth, matsize, w_count):
    left = 1
    right = START_DISTANCE
    while right - left > 0:
        mid = (left + right)//2
        result = function(distance=mid, bitwidth=bitwidth, matsize=matsize, 
                          use_nops=True, 
                          setup_rws=[f"RW {w}" for w in range(4, w_count+4)])
        if result:
            right = mid
        else:
            left = mid+1
...
# find the mimimum viable distance for a variety of bitwidths, matsizes, and 
# weights to add to the queue in the setup.
# return a nested dictionary that holds the minimum viable distance for each
# combination of bitwidth and matsize. accessed as dist = d[bitwidth][matsize][setup queue fill].
def min_viable_distance_all(function, bitwidths, matsizes, w_counts):
    d = {}
    for bitwidth in bitwidths:
        ...
        for matsize in matsizes:
            ...
            d[b_index][m_index] = {}
            for w_count in w_counts:
                w_index = "w" + str(w_count)
                distance = min_viable_distance_fixed(function, bitwidth, 
                                                     matsize, w_count)
                d[b_index][m_index][w_index] = distance
    return d

# compare the control test with the no-NOP version for all bitwidths, matsizes,
# and weights to add to the queue in the setup. return a nested dictionary that
# holds the result for each combination of bitwidth and matsize. accessed as 
# result = d[bitwidth][matsize][setup queue fill].
def no_nop_comparison_all(squishtest, bitwidths, matsizes, w_counts):
    d = {}
    for bitwidth in bitwidths:
        b_index = "b" + str(bitwidth)
        d[b_index] = {}
        for matsize in matsizes:
            ...
            for w_count in w_counts:
                w_index = "w" + str(w_count)
                result = squishtest(distance=START_DISTANCE, bitwidth=bitwidth, 
                                    matsize=matsize, use_nops=False, 
                                    setup_rws=[f"RW {w}" for w in range(4, w_count+4)])
                d[b_index][m_index][w_index] = result
    return d
...
# HM5 = HM2 * RW1.
def test_mmc_switch_behavior(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 1 1 1", "RHM 2 2 1",
                       ...
                       name=f"test_mmc_switch_behavior_w{len(setup_rws)}",
...
def test_all_rhm_rhm(test_function, bitwidths, matsizes):
    return {
        "rhm_rhm_diff_hm_diff_ub": test_function(rhm_rhm_diff_hm_diff_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "rhm_rhm_diff_hm_same_ub": test_function(rhm_rhm_diff_hm_same_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "rhm_rhm_same_hm_diff_ub": test_function(rhm_rhm_same_hm_diff_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "rhm_rhm_same_hm_same_ub": test_function(rhm_rhm_same_hm_same_ub, bitwidths, matsizes, [0, 1, 2, 3, 4])
    }
...
def rhm_rhm_diff_hm_diff_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 2 0 1", "RHM 3 1 1"],
                       instrs=[f"RHM 0 0 {L1}", f"RHM 1 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rhm_rhm_diff_hm_diff_ub_w{len(setup_rws)}", 
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def rhm_rhm_diff_hm_same_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws,
                       instrs=[f"RHM 0 0 {L1}", f"RHM 1 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rhm_rhm_diff_hm_same_ub_w{len(setup_rws)}", 
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def rhm_rhm_same_hm_diff_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws,
                       instrs=[f"RHM 0 0 {L1}", f"RHM 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rhm_rhm_same_hm_diff_ub_w{len(setup_rws)}", 
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def rhm_rhm_same_hm_same_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws,
                       instrs=[f"RHM 0 0 {L1}", f"RHM 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rhm_rhm_same_hm_same_ub_w{len(setup_rws)}", 
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_rhm_whm(test_function, bitwidths, matsizes):
    return {
        "rhm_whm_diff_hm_diff_ub": test_function(rhm_whm_diff_hm_diff_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "rhm_whm_diff_hm_same_ub": test_function(rhm_whm_diff_hm_same_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "rhm_whm_same_hm_diff_ub": test_function(rhm_whm_same_hm_diff_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "rhm_whm_same_hm_same_ub": test_function(rhm_whm_same_hm_same_ub, bitwidths, matsizes, [0, 1, 2, 3, 4])
    }
...
def rhm_whm_diff_hm_diff_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 2 1 1"],
                       instrs=[f"RHM 0 0 {L1}", f"WHM 1 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rhm_whm_diff_hm_diff_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def rhm_whm_diff_hm_same_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws,
                       instrs=[f"RHM 0 0 {L1}", f"WHM 1 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rhm_whm_diff_hm_same_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def rhm_whm_same_hm_diff_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 2 1 1"],
                       instrs=[f"RHM 0 0 {L1}", f"WHM 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rhm_whm_same_hm_diff_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def rhm_whm_same_hm_same_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1"],
                       instrs=[f"RHM 0 0 {L1}", f"WHM 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rhm_whm_same_hm_same_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_rhm_rw(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 4 because instr2 is an RW
        "rhm_rw": test_function(rhm_rw, bitwidths, matsizes, [0, 1, 2, 3])
    }

# from HM0 to UB0 and reading from RW0
# setup is not needed.
# instrs write HM 0 to UB 0 and then loads a weight into RW 0.
def rhm_rw(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws,
                       instrs=[f"RHM 0 0 {L1}", f"RW 0"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rhm_rw_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_rhm_mmc(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 0 because instr2 is an MMC
        "rhm_mmc_same_ub_no_s": test_function(rhm_mmc_same_ub_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "rhm_mmc_diff_ub_no_s": test_function(rhm_mmc_diff_ub_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "rhm_mmc_same_ub_yes_s": test_function(rhm_mmc_same_ub_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "rhm_mmc_diff_ub_yes_s": test_function(rhm_mmc_diff_ub_yes_s, bitwidths, matsizes, [1, 2, 3, 4])
    }
...
def rhm_mmc_same_ub_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws,
                       instrs=[f"RHM 0 0 {L1}", f"MMC 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rhm_mmc_same_ub_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def rhm_mmc_diff_ub_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 1 1"],
                       instrs=[f"RHM 0 0 {L1}", f"MMC 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rhm_mmc_diff_ub_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def rhm_mmc_same_ub_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws,
                       instrs=[f"RHM 0 0 {L1}", f"MMC.S 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rhm_mmc_same_ub_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def rhm_mmc_diff_ub_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 1 1"],
                       instrs=[f"RHM 0 0 {L1}", f"MMC.S 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rhm_mmc_diff_ub_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_rhm_act(test_function, bitwidths, matsizes):
    return {
        "rhm_act_same_ub": test_function(rhm_act_same_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "rhm_act_diff_ub": test_function(rhm_act_diff_ub, bitwidths, matsizes, [0, 1, 2, 3, 4])
    }
...
#     existing data.
def rhm_act_same_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=["RHM 1 1 1", "RW 2", "MMC.S 0 1 1"] + setup_rws,
                       instrs=[f"RHM 0 0 {L1}", f"ACT 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rhm_act_same_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def rhm_act_diff_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=["RHM 1 1 1", "RW 2", "MMC.S 0 1 1"] + setup_rws,
                       instrs=[f"RHM 0 0 {L1}", f"ACT 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rhm_act_diff_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_rhm_hlt(test_function, bitwidths, matsizes):
    return {
        "rhm_hlt": test_function(rhm_hlt, bitwidths, matsizes, [0, 1, 2, 3, 4])
    }

# from HM0 to UB0 and halting
# setup is not needed.
# instrs write HM 0 to UB 0 and then halt.
def rhm_hlt(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws,
                       instrs=[f"RHM 0 0 {L1}", f"HLT"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rhm_hlt_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_whm_rhm(test_function, bitwidths, matsizes):
    return {
        "whm_rhm_diff_hm_diff_ub": test_function(whm_rhm_diff_hm_diff_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "whm_rhm_diff_hm_same_ub": test_function(whm_rhm_diff_hm_same_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "whm_rhm_same_hm_diff_ub": test_function(whm_rhm_same_hm_diff_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "whm_rhm_same_hm_same_ub": test_function(whm_rhm_same_hm_same_ub, bitwidths, matsizes, [0, 1, 2, 3, 4])
    }
...
def whm_rhm_diff_hm_diff_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 2 0 1"],
                       instrs=[f"WHM 0 0 {L1}", f"RHM 1 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"whm_rhm_diff_hm_diff_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def whm_rhm_diff_hm_same_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 2 0 1"],
                       instrs=[f"WHM 0 0 {L1}", f"RHM 1 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"whm_rhm_diff_hm_same_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def whm_rhm_same_hm_diff_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1"],
                       instrs=[f"WHM 0 0 {L1}", f"RHM 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"whm_rhm_same_hm_diff_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def whm_rhm_same_hm_same_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1"],
                       instrs=[f"WHM 0 0 {L1}", f"RHM 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"whm_rhm_same_hm_same_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_whm_whm(test_function, bitwidths, matsizes):
    return {
        "whm_whm_diff_hm_diff_ub": test_function(whm_whm_diff_hm_diff_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "whm_whm_same_hm_diff_ub": test_function(whm_whm_same_hm_diff_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "whm_whm_diff_hm_same_ub": test_function(whm_whm_diff_hm_same_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "whm_whm_same_hm_same_ub": test_function(whm_whm_same_hm_same_ub, bitwidths, matsizes, [0, 1, 2, 3, 4])
    }
...
def whm_whm_diff_hm_diff_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 2 0 1", "RHM 3 1 1"],
                       instrs=[f"WHM 0 0 {L1}", f"WHM 1 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"whm_whm_diff_hm_diff_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def whm_whm_same_hm_diff_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1", "RHM 2 1 1"],
                       instrs=[f"WHM 0 0 {L1}", f"WHM 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"whm_whm_same_hm_diff_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def whm_whm_diff_hm_same_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 2 0 1"],
                       instrs=[f"WHM 0 0 {L1}", f"WHM 1 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"whm_whm_diff_hm_same_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def whm_whm_same_hm_same_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1"],
                       instrs=[f"WHM 0 0 {L1}", f"WHM 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"whm_whm_same_hm_same_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_whm_rw(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 4 because instr2 is an RW
        "whm_rw": test_function(whm_rw, bitwidths, matsizes, [0, 1, 2, 3])
    }
...
def whm_rw(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1", "RHM 2 1 1"],
                       instrs=[f"WHM 0 0 {L1}", "RW 0"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"whm_rw_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_whm_mmc(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 0 because instr2 is an MMC
        "whm_mmc_same_ub_no_s": test_function(whm_mmc_same_ub_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "whm_mmc_diff_ub_no_s": test_function(whm_mmc_diff_ub_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "whm_mmc_same_ub_yes_s": test_function(whm_mmc_same_ub_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "whm_mmc_diff_ub_yes_s": test_function(whm_mmc_diff_ub_yes_s, bitwidths, matsizes, [1, 2, 3, 4])
    }
...
def whm_mmc_same_ub_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1"],
                       instrs=[f"WHM 0 0 {L1}", f"MMC 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"whm_mmc_same_ub_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def whm_mmc_diff_ub_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 1 1"],
                       instrs=[f"WHM 0 0 {L1}", f"MMC 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"whm_mmc_diff_ub_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
#     the weight queue.
# instrs write from UB0 to HM0 and then multiply UB0 into ACC0.
def whm_mmc_same_ub_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1"],
                       instrs=[f"WHM 0 0 {L1}", f"MMC.S 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"whm_mmc_same_ub_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
#     the weight queue.
# instrs write from UB0 to HM0 and then multiply UB1 into ACC0.
def whm_mmc_diff_ub_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 1 1"],
                       instrs=[f"WHM 0 0 {L1}", f"MMC.S 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"whm_mmc_diff_ub_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_whm_act(test_function, bitwidths, matsizes):
    return {
        "whm_act_same_ub": test_function(whm_act_same_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "whm_act_diff_ub": test_function(whm_act_diff_ub, bitwidths, matsizes, [0, 1, 2, 3, 4])
    }
...
def whm_act_same_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=["RHM 1 1 1", "RW 2", "MMC.S 0 1 1"] + setup_rws,
                       instrs=[f"WHM 0 0 {L1}", f"ACT 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"whm_act_same_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def whm_act_diff_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=["RHM 3 0 1", "RHM 2 2 1", "RW 2", "MMC.S 0 2 1"]
                            + setup_rws,
                       instrs=[f"WHM 0 0 {L1}", f"ACT 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"whm_act_diff_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_whm_hlt(test_function, bitwidths, matsizes):
    return {
        "whm_hlt": test_function(whm_hlt, bitwidths, matsizes, [0, 1, 2, 3, 4])
    }
...
def whm_hlt(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1"],
                       instrs=[f"WHM 0 0 {L1}", "HLT"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"whm_hlt_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_rw_rhm(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 4 because instr1 is an RW
        "rw_rhm": test_function(rw_rhm, bitwidths, matsizes, [0, 1, 2, 3])
    }

# reading from RW0 and moving from HM0 to UB0
# setup not needed.
# instrs load a weight into RW0 and then move a matrix from HM0 to UB0
def rw_rhm(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws,
                       instrs=["RW 0", f"RHM 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rw_rhm_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_rw_whm(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 4 because instr1 is an RW
        "rw_whm": test_function(rw_whm, bitwidths, matsizes, [0, 1, 2, 3])
    }
...
def rw_whm(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1"],
                       instrs=["RW 0", f"WHM 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rw_whm_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_rw_rw(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 3 or 4 because instr1 and instr2 are RWs
        "rw_rw_same_weights": test_function(rw_rw_same_weights, bitwidths, matsizes, [0, 1, 2]),
        "rw_rw_diff_weights": test_function(rw_rw_diff_weights, bitwidths, matsizes, [0, 1, 2]),
    }

# reading from RW0 and RW0, buffer starts empty (same weights)
# setup not needed.
# instrs load RW0 into the weight queue and then load RW0 again.
def rw_rw_same_weights(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws,
                       instrs=["RW 0", "RW 0"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rw_rw_same_weights_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def rw_rw_diff_weights(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws,
                       instrs=["RW 0", "RW 1"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rw_rw_diff_weights_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_rw_mmc(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 4 because instr1 is an RW
        "rw_mmc_no_s": test_function(rw_mmc_no_s, bitwidths, matsizes, [0, 1, 2, 3]),
        "rw_mmc_yes_s": test_function(rw_mmc_yes_s, bitwidths, matsizes, [0, 1, 2, 3]),
    }
...
def rw_mmc_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=["RW 0", f"MMC 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rw_mmc_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
# setup reads HM0 into UB0.
# instrs load RW0 into the weight queue and then multiply UB0 and RW0 into ACC0.
def rw_mmc_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=["RW 0", f"MMC.S 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rw_mmc_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_rw_act(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 4 because instr1 is an RW
        "rw_act": test_function(rw_act, bitwidths, matsizes, [0, 1, 2, 3])
    }
...
def rw_act(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=["RHM 1 1 1", "RW 2", "MMC.S 0 1 1"] + setup_rws,
                       instrs=["RW 0", f"ACT 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rw_act_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_rw_hlt(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 4 because instr1 is an RW
        "rw_hlt": test_function(rw_hlt, bitwidths, matsizes, [0, 1, 2, 3]),
    }

# reading from RW0 and halting, buffer starts empty
# setup not needed.
# instrs load RW0 into the weight queue and then halt.
def rw_hlt(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws,
                       instrs=["RW 0", "HLT"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"rw_hlt_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_mmc_rhm(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 0 because instr1 is an MMC
        "mmc_rhm_same_ub_no_s": test_function(mmc_rhm_same_ub_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_rhm_same_ub_yes_s": test_function(mmc_rhm_same_ub_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_rhm_diff_ub_no_s": test_function(mmc_rhm_diff_ub_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_rhm_diff_ub_yes_s": test_function(mmc_rhm_diff_ub_yes_s, bitwidths, matsizes, [1, 2, 3, 4])
    }
...
def mmc_rhm_same_ub_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1"],
                       instrs=[f"MMC 0 0 {L1}", f"RHM 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_rhm_same_ub_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
# setup reads HM1 into UB0 so that it has a value, loads RW0, and loads RW1.
# instrs multiply UB0 with RW0 into ACC0 and then move a matrix from HM0 to UB0.
def mmc_rhm_same_ub_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1"],
                       instrs=[f"MMC.S 0 0 {L1}", f"RHM 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_rhm_same_ub_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def mmc_rhm_diff_ub_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1"],
                       instrs=[f"MMC 0 0 {L1}", f"RHM 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_rhm_diff_ub_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
# setup reads HM1 into UB0 so that it has a value, loads RW0, and loads RW1.
# instrs multiply HM0 with RW0 into ACC0 and then move a matrix from HM0 to UB1.
def mmc_rhm_diff_ub_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1"],
                       instrs=[f"MMC.S 0 0 {L1}", f"RHM 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_rhm_diff_ub_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_mmc_whm(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 0 because instr1 is an MMC
        "mmc_whm_same_ub_no_s": test_function(mmc_whm_same_ub_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_whm_same_ub_yes_s": test_function(mmc_whm_same_ub_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_whm_diff_ub_no_s": test_function(mmc_whm_diff_ub_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_whm_diff_ub_yes_s": test_function(mmc_whm_diff_ub_yes_s, bitwidths, matsizes, [1, 2, 3, 4])
    }
...
def mmc_whm_same_ub_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1"],
                       instrs=[f"MMC 0 0 {L1}", f"WHM 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_whm_same_ub_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
# setup reads HM1 into UB0 so that it has a different value, loads RW0 and RW1.
# instrs multiply UB0 with RW0 into ACC0 and then move a matrix from UB0 to HM0.
def mmc_whm_same_ub_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1"],
                       instrs=[f"MMC.S 0 0 {L1}", f"WHM 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_whm_same_ub_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def mmc_whm_diff_ub_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1", "RHM 2 1 1"],
                       instrs=[f"MMC 0 0 {L1}", f"WHM 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_whm_diff_ub_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
# setup reads HM1 into UB0 and HM2 into UB1 and loads RW0 and RW1.
# instrs multiply UB0 with RW0 into ACC0 and then move a matrix from UB1 to HM0.
def mmc_whm_diff_ub_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 1 0 1", "RHM 2 1 1"],
                       instrs=[f"MMC.S 0 0 {L1}", f"WHM 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_whm_diff_ub_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_mmc_rw(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 0 because instr1 is an MMC
        # can't do queue fill 4 because instr2 is an RW and instr1 doesn't use 
        # up a weight
        "mmc_rw_no_s": test_function(mmc_rw_no_s, bitwidths, matsizes, [1, 2, 3]),
        # can't do queue fill 0 because instr1 is an MMC
        "mmc_rw_yes_s": test_function(mmc_rw_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
    }
...
def mmc_rw_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=[f"MMC 0 0 {L1}", "RW 0"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_rw_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
# instrs multiply UB0 with the fifo queue into ACC0 when there's nothing in the 
#     FIFO queue. Then load RW0 into the FIFO queue.
def mmc_rw_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=[f"MMC.S 0 0 {L1}", "RW 0"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_rw_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_mmc_mmc(test_function, bitwidths, matsizes):
    return {
        # first mmc no_s can't do queue fill 0 because instr1 is an MMC
        # first mmc yes_s can't do queue fill 0 because instr1 is an MMC
        # first mmc yes_s can't do queue fill 1 because instr1 is an MMC that 
        # switches weights, and this leaves nothing for the second mmc
        "mmc_mmc_diff_ub_diff_acc_no_s_no_s": test_function(mmc_mmc_diff_ub_diff_acc_no_s_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_mmc_diff_ub_diff_acc_no_s_yes_s": test_function(mmc_mmc_diff_ub_diff_acc_no_s_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_mmc_diff_ub_diff_acc_yes_s_no_s": test_function(mmc_mmc_diff_ub_diff_acc_yes_s_no_s, bitwidths, matsizes, [2, 3, 4]),
        "mmc_mmc_diff_ub_diff_acc_yes_s_yes_s": test_function(mmc_mmc_diff_ub_diff_acc_yes_s_yes_s, bitwidths, matsizes, [2, 3, 4]),
        "mmc_mmc_same_ub_diff_acc_no_s_no_s": test_function(mmc_mmc_same_ub_diff_acc_no_s_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_mmc_same_ub_diff_acc_no_s_yes_s": test_function(mmc_mmc_same_ub_diff_acc_no_s_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_mmc_same_ub_diff_acc_yes_s_no_s": test_function(mmc_mmc_same_ub_diff_acc_yes_s_no_s, bitwidths, matsizes, [2, 3, 4]),
        "mmc_mmc_same_ub_diff_acc_yes_s_yes_s": test_function(mmc_mmc_same_ub_diff_acc_yes_s_yes_s, bitwidths, matsizes, [2, 3, 4]),
        "mmc_mmc_diff_ub_same_acc_no_s_no_s": test_function(mmc_mmc_diff_ub_same_acc_no_s_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_mmc_diff_ub_same_acc_no_s_yes_s": test_function(mmc_mmc_diff_ub_same_acc_no_s_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_mmc_diff_ub_same_acc_yes_s_no_s": test_function(mmc_mmc_diff_ub_same_acc_yes_s_no_s, bitwidths, matsizes, [2, 3, 4]),
        "mmc_mmc_diff_ub_same_acc_yes_s_yes_s": test_function(mmc_mmc_diff_ub_same_acc_yes_s_yes_s, bitwidths, matsizes, [2, 3, 4]),
        "mmc_mmc_same_ub_same_acc_no_s_no_s": test_function(mmc_mmc_same_ub_same_acc_no_s_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_mmc_same_ub_same_acc_no_s_yes_s": test_function(mmc_mmc_same_ub_same_acc_no_s_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_mmc_same_ub_same_acc_yes_s_no_s": test_function(mmc_mmc_same_ub_same_acc_yes_s_no_s, bitwidths, matsizes, [2, 3, 4]),
        "mmc_mmc_same_ub_same_acc_yes_s_yes_s": test_function(mmc_mmc_same_ub_same_acc_yes_s_yes_s, bitwidths, matsizes, [2, 3, 4]),
    }
...
#     (no S).
def mmc_mmc_diff_ub_diff_acc_no_s_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 1 1 1"],
                       instrs=[f"MMC 0 0 {L1}", f"MMC 1 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_mmc_diff_ub_diff_acc_no_s_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
# instrs multiply UB0 with RW0 into ACC0 (no S) and UB1 with RW1 into ACC1
#     (with S).
def mmc_mmc_diff_ub_diff_acc_no_s_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 1 1 1"],
                       instrs=[f"MMC 0 0 {L1}", f"MMC.S 1 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_mmc_diff_ub_diff_acc_no_s_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
#     (no S).
def mmc_mmc_diff_ub_diff_acc_yes_s_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 1 1 1"],
                       instrs=[f"MMC.S 0 0 {L1}", f"MMC 1 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_mmc_diff_ub_diff_acc_yes_s_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
# instrs multiply UB0 with RW0 into ACC0 (with S) and UB1 with RW1 into ACC1
#     (with S).
def mmc_mmc_diff_ub_diff_acc_yes_s_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 1 1 1"],
                       instrs=[f"MMC.S 0 0 {L1}", f"MMC.S 1 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_mmc_diff_ub_diff_acc_yes_s_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
#     (no S).
def mmc_mmc_same_ub_diff_acc_no_s_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=[f"MMC 0 0 {L1}", f"MMC 1 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_mmc_same_ub_diff_acc_no_s_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
# instrs multiply UB0 with RW0 into ACC0 (no S) and UB0 with RW0 into ACC1 
#     (w/ S).
def mmc_mmc_same_ub_diff_acc_no_s_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=[f"MMC 0 0 {L1}", f"MMC.S 1 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_mmc_same_ub_diff_acc_no_s_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
#     (no S).
def mmc_mmc_same_ub_diff_acc_yes_s_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=[f"MMC.S 0 0 {L1}", f"MMC 1 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_mmc_same_ub_diff_acc_yes_s_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
# instrs multiply UB0 with RW0 into ACC0 (w/ S) and UB0 with RW1 into ACC1 
#     (w/ S).
def mmc_mmc_same_ub_diff_acc_yes_s_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=[f"MMC.S 0 0 {L1}", f"MMC.S 1 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_mmc_same_ub_diff_acc_yes_s_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
#     (no S).
def mmc_mmc_diff_ub_same_acc_no_s_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 1 1 1"],
                       instrs=[f"MMC 0 0 {L1}", f"MMC 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_mmc_diff_ub_same_acc_no_s_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
# instrs multiply UB0 with RW0 into ACC0 (no S) and UB1 with RW0 into ACC0
#     (w/ S).
def mmc_mmc_diff_ub_same_acc_no_s_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 1 1 1"],
                       instrs=[f"MMC 0 0 {L1}", f"MMC.S 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_mmc_diff_ub_same_acc_no_s_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
#     (no S).
def mmc_mmc_diff_ub_same_acc_yes_s_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 1 1 1"],
                       instrs=[f"MMC.S 0 0 {L1}", f"MMC 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_mmc_diff_ub_same_acc_yes_s_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
# instrs multiply UB0 with RW0 into ACC0 (w/ S) and UB1 with RW1 into ACC0
#     (w/ S).
def mmc_mmc_diff_ub_same_acc_yes_s_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 1 1 1"],
                       instrs=[f"MMC.S 0 0 {L1}", f"MMC.S 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_mmc_diff_ub_same_acc_yes_s_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
#     (no S).
def mmc_mmc_same_ub_same_acc_no_s_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=[f"MMC 0 0 {L1}", f"MMC 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_mmc_same_ub_same_acc_no_s_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
# instrs multiply UB0 with RW0 into ACC0 (no S) and UB0 with RW0 into ACC0
#     (w/ S).
def mmc_mmc_same_ub_same_acc_no_s_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=[f"MMC 0 0 {L1}", f"MMC.S 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_mmc_same_ub_same_acc_no_s_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
#     (no S).
def mmc_mmc_same_ub_same_acc_yes_s_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=[f"MMC.S 0 0 {L1}", f"MMC 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_mmc_same_ub_same_acc_yes_s_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
# instrs multiply UB0 with RW0 into ACC0 (w/ S) and UB0 with RW1 into ACC0
#     (w/ S).
def mmc_mmc_same_ub_same_acc_yes_s_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=[f"MMC.S 0 0 {L1}", f"MMC.S 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_mmc_same_ub_same_acc_yes_s_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_mmc_act(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 0 because instr1 is an MMC
        "mmc_act_diff_ub_diff_acc_no_s": test_function(mmc_act_diff_ub_diff_acc_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_act_diff_ub_diff_acc_yes_s": test_function(mmc_act_diff_ub_diff_acc_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_act_same_ub_diff_acc_no_s": test_function(mmc_act_same_ub_diff_acc_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_act_same_ub_diff_acc_yes_s": test_function(mmc_act_same_ub_diff_acc_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_act_diff_ub_same_acc_no_s": test_function(mmc_act_diff_ub_same_acc_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_act_diff_ub_same_acc_yes_s": test_function(mmc_act_diff_ub_same_acc_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_act_same_ub_same_acc_no_s": test_function(mmc_act_same_ub_same_acc_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_act_same_ub_same_acc_yes_s": test_function(mmc_act_same_ub_same_acc_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
    }
...
#     ACC1 to UB1.
def mmc_act_diff_ub_diff_acc_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 3 3 1", "MMC 1 3 1"],
                       instrs=[f"MMC 0 0 {L1}", f"ACT 1 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_act_diff_ub_diff_acc_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
#     from ACC1 to UB1.
def mmc_act_diff_ub_diff_acc_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 3 3 1", "MMC 1 3 1"],
                       instrs=[f"MMC.S 0 0 {L1}", f"ACT 1 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_act_diff_ub_diff_acc_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
#     from ACC1 to UB0.
def mmc_act_same_ub_diff_acc_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 2 2 1", "MMC 1 2 1"],
                       instrs=[f"MMC 0 0 {L1}", f"ACT 1 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_act_same_ub_diff_acc_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
# instrs multiply UB0 with RW0 into ACC0 (w/ S) and activate the setup result
#     from ACC1 to UB0.
def mmc_act_same_ub_diff_acc_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 2 2 1", "MMC 1 2 1"],
                       instrs=[f"MMC.S 0 0 {L1}", f"ACT 1 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_act_same_ub_diff_acc_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def mmc_act_diff_ub_same_acc_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=[f"MMC 0 0 {L1}", f"ACT 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_act_diff_ub_same_acc_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def mmc_act_diff_ub_same_acc_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=[f"MMC.S 0 0 {L1}", f"ACT 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_act_diff_ub_same_acc_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def mmc_act_same_ub_same_acc_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=[f"MMC 0 0 {L1}", f"ACT 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_act_same_ub_same_acc_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def mmc_act_same_ub_same_acc_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=[f"MMC.S 0 0 {L1}", f"ACT 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_act_same_ub_same_acc_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_mmc_hlt(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 0 because instr1 is an MMC
        "mmc_hlt_no_s": test_function(mmc_hlt_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "mmc_hlt_yes_s": test_function(mmc_hlt_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
    }
...
def mmc_hlt_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=[f"MMC 0 0 {L1}", "HLT"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_hlt_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
# instrs multiply UB0 with the fifo queue into ACC0 when there's nothing in the 
#     FIFO queue. Then halt.
def mmc_hlt_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1"],
                       instrs=[f"MMC.S 0 0 {L1}", "HLT"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"mmc_hlt_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_act_rhm(test_function, bitwidths, matsizes):
    return {
        "act_rhm_same_ub": test_function(act_rhm_same_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "act_rhm_diff_ub": test_function(act_rhm_diff_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
    }
...
def act_rhm_same_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=["RHM 1 1 1", "RW 2", "MMC.S 0 1 1"] + setup_rws,
                       instrs=[f"ACT 0 0 {L1}", f"RHM 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_rhm_same_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def act_rhm_diff_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=["RHM 2 2 1", "RW 2", "MMC.S 0 2 1"] + setup_rws,
                       instrs=[f"ACT 0 0 {L1}", f"RHM 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_rhm_diff_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_act_whm(test_function, bitwidths, matsizes):
    return {
        "act_whm_same_ub": test_function(act_whm_same_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "act_whm_diff_ub": test_function(act_whm_diff_ub, bitwidths, matsizes, [0, 1, 2, 3, 4]),
    }
...
def act_whm_same_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=["RHM 1 1 1", "RW 2", "MMC.S 0 1 1"] + setup_rws,
                       instrs=[f"ACT 0 0 {L1}", f"WHM 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_whm_same_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def act_whm_diff_ub(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=["RHM 1 1 1", "RHM 2 2 1", "RW 2", "MMC.S 0 2 1"]
                            + setup_rws,
                       instrs=[f"ACT 0 0 {L1}", f"WHM 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_whm_diff_ub_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_act_rw(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 4 because instr2 is an RW
        "act_rw": test_function(act_rw, bitwidths, matsizes, [0, 1, 2, 3]),
    }
...
def act_rw(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=["RHM 1 1 1", "RW 2", "MMC.S 0 1 1"] + setup_rws,
                       instrs=[f"ACT 0 0 {L1}", "RW 0"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_rw_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_act_mmc(test_function, bitwidths, matsizes):
    return {
        # can't do queue fill 0 because instr2 is an MMC
        "act_mmc_diff_ub_diff_acc_no_s": test_function(act_mmc_diff_ub_diff_acc_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "act_mmc_diff_ub_diff_acc_yes_s": test_function(act_mmc_diff_ub_diff_acc_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "act_mmc_same_ub_diff_acc_no_s": test_function(act_mmc_same_ub_diff_acc_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "act_mmc_same_ub_diff_acc_yes_s": test_function(act_mmc_same_ub_diff_acc_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "act_mmc_diff_ub_same_acc_no_s": test_function(act_mmc_diff_ub_same_acc_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "act_mmc_diff_ub_same_acc_yes_s": test_function(act_mmc_diff_ub_same_acc_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "act_mmc_same_ub_same_acc_no_s": test_function(act_mmc_same_ub_same_acc_no_s, bitwidths, matsizes, [1, 2, 3, 4]),
        "act_mmc_same_ub_same_acc_yes_s": test_function(act_mmc_same_ub_same_acc_yes_s, bitwidths, matsizes, [1, 2, 3, 4]),
    }
...
def act_mmc_diff_ub_diff_acc_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 1 1 1", "MMC 0 0 1"],
                       instrs=[f"ACT 0 0 {L1}", f"MMC 1 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_mmc_diff_ub_diff_acc_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def act_mmc_diff_ub_diff_acc_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 1 1 1", "MMC 0 0 1"],
                       instrs=[f"ACT 0 0 {L1}", f"MMC.S 1 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_mmc_diff_ub_diff_acc_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def act_mmc_same_ub_diff_acc_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "MMC 0 0 1"],
                       instrs=[f"ACT 0 0 {L1}", f"MMC 1 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_mmc_same_ub_diff_acc_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def act_mmc_same_ub_diff_acc_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "MMC 0 0 1"],
                       instrs=[f"ACT 0 0 {L1}", f"MMC.S 1 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_mmc_same_ub_diff_acc_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def act_mmc_diff_ub_same_acc_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 1 1 1", "MMC 0 0 1"],
                       instrs=[f"ACT 0 0 {L1}", f"MMC 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_mmc_diff_ub_same_acc_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def act_mmc_diff_ub_same_acc_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "RHM 1 1 1", "MMC 0 0 1"],
                       instrs=[f"ACT 0 0 {L1}", f"MMC.S 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_mmc_diff_ub_same_acc_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def act_mmc_same_ub_same_acc_no_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "MMC 0 0 1"],
                       instrs=[f"ACT 0 0 {L1}", f"MMC 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_mmc_same_ub_same_acc_no_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def act_mmc_same_ub_same_acc_yes_s(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=setup_rws + ["RHM 0 0 1", "MMC 0 0 1"],
                       instrs=[f"ACT 0 0 {L1}", f"MMC.S 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_mmc_same_ub_same_acc_yes_s_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_act_act(test_function, bitwidths, matsizes):
    return {
        "act_act_diff_ub_diff_acc": test_function(act_act_diff_ub_diff_acc, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "act_act_same_ub_diff_acc": test_function(act_act_same_ub_diff_acc, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "act_act_diff_ub_same_acc": test_function(act_act_diff_ub_same_acc, bitwidths, matsizes, [0, 1, 2, 3, 4]),
        "act_act_same_ub_same_acc": test_function(act_act_same_ub_same_acc, bitwidths, matsizes, [0, 1, 2, 3, 4]),
    }
...
def act_act_diff_ub_diff_acc(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=["RHM 0 0 1", "RHM 1 1 1", "RW 2", "RW 3", 
                              "MMC.S 0 0 1", "MMC.S 1 1 1"] + setup_rws,
                       instrs=[f"ACT 0 0 {L1}", f"ACT 1 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_act_diff_ub_diff_acc_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def act_act_same_ub_diff_acc(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=["RHM 0 0 1", "RHM 1 1 1", "RW 2", "RW 3", 
                              "MMC.S 0 0 1", "MMC.S 1 1 1"] + setup_rws,
                       instrs=[f"ACT 0 0 {L1}", f"ACT 1 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_act_same_ub_diff_acc_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def act_act_diff_ub_same_acc(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=["RHM 0 0 1", "RW 2", "MMC.S 0 0 1"] + setup_rws,
                       instrs=[f"ACT 0 0 {L1}", f"ACT 0 1 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_act_diff_ub_same_acc_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def act_act_same_ub_same_acc(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=["RHM 0 0 1", "RW 2", "MMC.S 0 0 1"] + setup_rws,
                       instrs=[f"ACT 0 0 {L1}", f"ACT 0 0 {L2}"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_act_same_ub_same_acc_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
...
def test_all_act_hlt(test_function, bitwidths, matsizes):
    return {
        "act_hlt": test_function(act_hlt, bitwidths, matsizes, [0, 1, 2, 3, 4]),
    }
...
def act_hlt(distance, bitwidth, matsize, use_nops, setup_rws):
    return squish_test(setup=["RHM 1 1 1", "RW 2", "MMC.S 0 1 1"] + setup_rws,
                       instrs=[f"ACT 0 0 {L1}", "HLT"], cleanup=[],
                       distance=distance, bitwidth=bitwidth, matsize=matsize,
                       name=f"act_hlt_w{len(setup_rws)}",
                       reset=False, absoluteaddrs=False, test_folder=TEST_FOLDER,
                       ctrl_distance = START_DISTANCE, use_nops=use_nops)
