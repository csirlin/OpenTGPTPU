...
def rhm_rhm(p: Params):
    return squish_test(setup=p.setup_rws,
                       instrs=[f"RHM {p.l2} {p.l2} {p.l1}", 
                               f"RHM {p.hm2} {p.ub2} {p.l2}"],
                       distance=p.distance, bitwidth=p.bitwidth, matsize=p.matsize,
                       name=p.get_relative_filepath(), 
                       reset=False, absoluteaddrs=True, test_folder=p.test_folder,
                       ctrl_distance=p.base_distance, use_nops=p.use_nops)
...
def rhm_whm(p: Params):
    ...
    return squish_test(setup=p.setup_rws + [f"RHM {hm_max} {p.ub2} {p.l2}"],
                       instrs=[f"RHM {p.l2} {p.l2} {p.l1}", 
                               f"WHM {p.hm2} {p.ub2} {p.l2}"],
                       distance=p.distance, bitwidth=p.bitwidth, matsize=p.matsize,
                       name=p.get_relative_filepath(), # f"rhm_whm_hm{p.hm2}_ub{p.ub2}_w{len(p.setup_rws)}",
                       reset=False, absoluteaddrs=True, test_folder=p.test_folder,
                       ctrl_distance=p.base_distance, use_nops=p.use_nops)
...
def rhm_mmc(p: Params):
    return squish_test(setup=p.setup_rws + [f"RHM 0 {p.ub2} {p.l2}"],
...
def rhm_act(p: Params):
    ...
    return squish_test(setup=[f"RHM {p.l2+p.l1} {ub_max} {p.l2}", "RW 2", 
...
def whm_rhm(p: Params):
    hm_max = max(p.l2+p.l1, p.hm2+p.l2)
    return squish_test(setup=p.setup_rws + [f"RHM {hm_max} {p.l2} {p.l1}"],
...
# instr2: RW 0
def whm_rw(p: Params):
    return squish_test(setup=p.setup_rws + [f"RHM {p.l1} 0 {p.l1}"],
...
def whm_act(p: Params):
    hm_max = p.l2+p.l1
    ub_max = max(p.l2+p.l1, p.ub2+p.l2)
    return squish_test(setup=[f"RHM {hm_max} {p.l2} {p.l1}", 
...
def rw_whm(p: Params):
    return squish_test(setup=p.setup_rws + [f"RHM {p.l2} 0 {p.l2}"],
...
def rw_mmc(p: Params):
    return squish_test(setup=p.setup_rws + [f"RHM 0 0 {p.l2}"],
...
def rw_act(p: Params):
    return squish_test(setup=[f"RHM 0 {p.l2} {p.l2}", "RW 2", 
                              f"MMC.S 0 {p.l2} {p.l2}"] + p.setup_rws,
...
def mmc_rhm(p: Params):
    return squish_test(setup=p.setup_rws + [f"RHM {p.l2} {p.l2} {p.l1}"],
...
# instr2: RW 0
def mmc_rw(p: Params):
    return squish_test(setup=p.setup_rws + [f"RHM 0 0 {p.l1}"],
...
def mmc_mmc(p: Params):
    return squish_test(setup=p.setup_rws + [f"RHM 0 {p.l2} {p.l1}", 
...
def act_rhm(p: Params):
    ub_max = max(p.l2+p.l1, p.ub2+p.l2)
    return squish_test(setup=[f"RHM {p.l2} {ub_max} {p.l1}", "RW 2", 
...
# instr2: RW 0
def act_rw(p: Params):
    return squish_test(setup=[f"RHM 0 {p.l1} {p.l1}", "RW 2", f"MMC.S 0 {p.l1} {p.l1}"]
                        + p.setup_rws,
...
def act_mmc(p: Params):
    ub_max = max(p.l2+p.l1, p.ub2+p.l2)
    return squish_test(setup=p.setup_rws + [f"RHM 0 {ub_max} {p.l1}", 
...
def act_act(p: Params):
    ub_max = max(p.l2+p.l1, p.ub2+p.l2)
    return squish_test(setup=[f"RHM 0 {ub_max} {p.l1}", "RW 2", 
                              f"MMC.S {p.l2} {ub_max} {p.l1}", 
...
def test_mmc_switch_behavior(p: Params):
    return squish_test(setup=p.setup_rws + ["RHM 0 0 1", "RHM 1 1 1", "RHM 2 2 1",
